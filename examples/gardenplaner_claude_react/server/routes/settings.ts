...
settingsRouter.get('/', (req: Request, res: Response) => {
  ...
  for (const row of rows) {
    settings[row.key] = { value: row.value, valueType: row.value_type, updatedAt: row.updated_at }
  }
  res.json(settings)
})
...
settingsRouter.put('/:key', (req: Request, res: Response) => {
  ...
  db.prepare(`INSERT INTO app_settings (user_id, key, value, value_type, updated_at) VALUES (?, ?, ?, ?, ?)
    ...
    .run(userId, key, String(value), valueType ?? 'string', ts)
  res.json({ key, value: String(value), valueType: valueType ?? 'string', updatedAt: ts })
})
